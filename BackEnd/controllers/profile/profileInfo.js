...
const profileInfo = async (req, res) => {
  try {
    ...
    let postUrl=await pool.query("select * from post where userid=$1",[req.userid]);
...
// console.log(postid.rows);
    ...
    res.status(200).json({ message: profileData.rows,postUrl:postUrl.rows ,taggedId: tagPost.rows });
  } catch (error) {
